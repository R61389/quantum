...
export function EngineeringLab() {
  return (
    <section id="laboratorio" className="relative scroll-mt-28 overflow-hidden py-28 lg:py-36">
      <div
        aria-hidden
        className="absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_0%,rgba(73,104,162,0.1),transparent)]"
      />
        ...
        <StaggerGroup className="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {processes.map(({ icon: Icon, title, description }, i) => (
            <StaggerItem key={title} className={i === 6 ? "lg:col-start-2" : undefined}>
              <div className="group relative h-full overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/[0.02] p-6">
                {/* scanning line accent */}
                <motion.div
                  aria-hidden
                  className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-quantum-navy to-transparent opacity-0 group-hover:opacity-100"
                  animate={{ x: ["-100%", "100%"] }}
...
function ScrollProof() {
  return (
    <motion.div
      ...
      className="mx-auto mt-14 flex max-w-3xl flex-wrap items-center justify-center gap-x-8 gap-y-4 rounded-2xl border border-foreground/10 bg-foreground/[0.02] px-8 py-6"
    >
      {proofPoints.map((point) => (
        <div key={point} className="flex items-center gap-2.5">
          <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-quantum-navy-light shadow-[0_0_8px_2px_rgba(73,104,162,0.6)]" />
