    ...
    return (
      <div>
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
          <p className="mb-2 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-quantum-electric-blue">
            <CheckCircle2 className="h-3.5 w-3.5" /> Batería Actual
          </p>
          <dl className="grid grid-cols-2 gap-y-1.5 text-sm">
            <dt className="text-white/50">Modelo</dt>
            <dd className="text-right text-white">{modelo || matched?.modelo || "No especificado"}</dd>
            <dt className="text-white/50">Voltaje</dt>
            <dd className="text-right text-white">{voltaje} V</dd>
            <dt className="text-white/50">Capacidad</dt>
            <dd className="text-right text-white">{ah} Ah</dd>
            <dt className="text-white/50">Energía</dt>
            <dd className="text-right text-white">{currentWh.toLocaleString("es-BO")} Wh</dd>
            {matched && (
              <>
                <dt className="text-white/50">Tecnología</dt>
                <dd className="text-right text-white">{matched.tipoQuimico}</dd>
                <dt className="text-white/50">Aplicación</dt>
                <dd className="text-right text-white">{matched.aplicacion.join(", ")}</dd>
              </>
            )}
          </dl>
          {identification.exact && (
            <p className="mt-3 text-xs text-quantum-lithium-green">
              ...
              return (
                <div key={label} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-quantum-electric-blue">{label}</span>
                    {option.benefitPercent > 0 && (
                      <span className="rounded-full bg-quantum-lithium-green/10 px-2 py-0.5 text-[0.65rem] text-quantum-lithium-green">
                        +{option.benefitPercent}% energía
                      </span>
                    )}
                  </div>
                  <p className="mt-1.5 text-sm font-semibold text-white">{option.product.modelo}</p>
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="space-y-4">
        <div>
          <label htmlFor="modelo" className="text-xs font-medium text-white/60">
            Modelo (opcional)
          </label>
          <input
            id="modelo"
            ...
            className="mt-1.5 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white placeholder:text-white/30 outline-none focus:border-primary/50"
          />
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="voltaje" className="text-xs font-medium text-white/60">
              Voltaje
            </label>
            <select
              id="voltaje"
          ...
          <div>
            <label htmlFor="ah" className="text-xs font-medium text-white/60">
              Capacidad (Ah)
            </label>
            <input
              id="ah"
        ...
        <div>
          <label className="text-xs font-medium text-white/60">Fotografía (opcional)</label>
          <label className="mt-1.5 flex cursor-pointer flex-col items-center gap-2 rounded-xl border border-dashed border-white/15 bg-black/20 px-4 py-6 text-center transition-colors hover:border-primary/40">
          ...
          <p className="mt-1.5 flex items-start gap-1.5 text-[0.7rem] leading-relaxed text-white/35">
      ...
      <button
        type="submit"
        className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-[linear-gradient(90deg,#00D4FF,#00FF88)] px-5 py-3 text-sm font-medium text-black"
      >
