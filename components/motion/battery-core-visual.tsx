...
export function BatteryCoreVisual() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[560px]" aria-hidden="true">
      ...
      <motion.div
        className="absolute inset-0 rounded-full border border-quantum-navy-light/35"
        animate={{ rotate: 360 }}
        transition={{ duration: 40, repeat: Infinity, ease: "linear" }}
      />
      <motion.div
        className="absolute inset-[8%] rounded-full border border-dashed border-quantum-navy-light/30"
        animate={{ rotate: -360 }}
        transition={{ duration: 55, repeat: Infinity, ease: "linear" }}
      />
      <motion.div
        className="absolute inset-[18%] rounded-full border border-quantum-navy-light/25"
        animate={{ rotate: 360 }}
      ...
      {/* pulsing core glow */}
      <motion.div
        className="absolute inset-[30%] rounded-full bg-quantum-navy-light/50 blur-3xl"
      ...
      {cells.map((cell) => (
        ...
        >
          <motion.div
            className="absolute left-1/2 top-0 h-10 w-6 -translate-x-1/2 rounded-md border border-quantum-navy-light/50 bg-gradient-to-b from-quantum-navy-light/40 to-quantum-navy-light/15 shadow-[0_0_18px_-2px_rgba(73,104,162,0.75)]"
      ...
      {particles.map((i) => {
        ...
        return (
          <motion.span
            key={i}
            className="absolute h-1 w-1 rounded-full bg-quantum-navy-light shadow-[0_0_8px_2px_rgba(73,104,162,0.8)]"
